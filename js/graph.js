...
export function createGraph(ui){
  ...
  function tooltipHTML(d){
    ...
    return `
      ...
      <div class="k">Keywords IA</div><div class="v">${chips(d.keywords, 18) || `<em style="color:var(--muted)">—</em>`}</div>
  ...
  function setSelected(id, links){
    selectedId = id || null;
    const adj = buildAdjacency(links);
    applySelectionHighlight(adj);
  }

  // sélection : niveau 1 + niveau 2
  ...
  function focusOnNode(id, nodes, links){
    if (!id) return;
    const node = nodes.find(n => n.id === id);
    if (!node) return;

    setSelected(id, links);
